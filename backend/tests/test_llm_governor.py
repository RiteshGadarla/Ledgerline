...
async def test_user_quota_is_checked_before_the_global_bucket(redis_client: redis.Redis) -> None:
    ...
    first = await governor.check_and_reserve(PRIMARY_MODEL, user_id="user-1")
    assert isinstance(first, Ok)

    second = await governor.check_and_reserve(PRIMARY_MODEL, user_id="user-1")
    assert isinstance(second, Err)
    assert "quota" in second.reason

    # a different user is unaffected by user-1's exhausted quota
    third = await governor.check_and_reserve(PRIMARY_MODEL, user_id="user-2")
...
async def test_rpd_limit_is_enforced_independently_of_rpm(redis_client: redis.Redis) -> None:
    governor = Governor(
        ...
        rpd_limits={PRIMARY_MODEL: 2, BACKUP_MODEL: 2},
        user_daily_quota=1000,
    )

    assert isinstance(await governor.check_and_reserve(PRIMARY_MODEL, user_id="u1"), Ok)
    assert isinstance(await governor.check_and_reserve(PRIMARY_MODEL, user_id="u2"), Ok)
    third = await governor.check_and_reserve(PRIMARY_MODEL, user_id="u3")
