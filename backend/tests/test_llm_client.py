...
async def test_fake_client_returns_fixture_and_records_calls() -> None:
    ...
    response = await client.generate(model="gemini-3.6-flash", prompt="match these", response_schema=Proposal)

    assert response.raw_json == fixture
    assert client.calls == [("gemini-3.6-flash", "match these")]


async def test_fake_client_raises_on_missing_fixture() -> None:
    client = FakeClient({})
    with pytest.raises(LlmUnavailable):
        await client.generate(model="gemini-3.6-flash", prompt="unrecorded prompt", response_schema=Proposal)
...
async def test_fake_client_rejects_malformed_fixture() -> None:
    ...
    with pytest.raises(Exception):  # noqa: B017 - pydantic's ValidationError, deliberately broad here
        await client.generate(model="gemini-3.6-flash", prompt="bad", response_schema=Proposal)
...
async def test_recording_client_writes_fixture_to_disk(tmp_path: Path) -> None:
    ...
    response = await recorder.generate(model="gemini-3.6-flash", prompt="match these", response_schema=Proposal)
